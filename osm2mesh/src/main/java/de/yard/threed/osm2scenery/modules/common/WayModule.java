...
/**
 * 13.2.26 Generic module for all kind of ways (roads (OSM  key "highway"), rivers, railroads)
 * Derived from HighwayModule.
...
public class WayModule extends SceneryModule {
        ...
        try {
            ...
            for (SceneryObject sceneryObject : roadsAndBridges.objects) {
                ...
                if (connectors.getFirst() != null) {
                    sfo.setStartConnector(connectors.getFirst());
                }
                if (connectors.getSecond() != null) {
                    sfo.setEndConnector(connectors.getSecond());
                }
                sfo.cca(/*new ArrayList<>(),*/ terrainMeshNotNeededOnceButNowNeeded, sceneryContext);
            ...
            terrainMeshNotNeededOnceButNowNeeded.meshService.addFailure(terrainMeshNotNeededOnceButNowNeeded.meshName, e.getMessage(), sourceRef,
    ...
    /**
     * Junctions/RoadConnector
     * Find already existing connector where the new way can attach or create new connector before creating the way.
     * We non assume creating all connector before the way is easier than modifying a way for adding a connector later.
     * Return start(0)/optionally mid cnnector, end connector(end of list).
     * A deadend will have no connector.
     *
     * @return
     */
    private static Pair<MeshPolygon/*MeshWayConnector*//*7.3.26SceneryWayConnector*/, MeshPolygon/*MeshWayConnector*//*7.3.26SceneryWayConnector*/> processConnectionCandidates(MapWaySegment2 mapWay, /*19.2.26SceneryObjectList roadsAndBridges,*/ SceneryContext sceneryContext, TerrainMesh terrainMesh) throws MeshInconsistencyException, OsmProcessException {
    ...
    /**
     * Returns the connector when the connector exists.
     ...
     * @return
     */
    private static MeshPolygon/*MeshWayConnector*//*7.3.26SceneryWayConnector*/ processConnectionCandidatesForOneNode(MapNode node, /*19.2.26SceneryObjectList roadsAndBridges, SceneryContext sceneryContext,*/ TerrainMesh terrainMesh, MapData mapdata) throws MeshInconsistencyException, OsmProcessException {
        ...
        //17.2.26 but now from terrainmesh
        //19.2.26 why this iteration? for (SceneryWayObject road : terrainMesh.getWays(ROAD)) {
    ...
    /**
     * Create a new connector/junction for a MapNode eine Junction oder RoadConnector erstellen, oder eine schon bestehende liefern.
     ...
     * 19.2.26: Now only persists and returns the connector but no longer adds to 'roadsAndBridges'
     *
     * @param
     * @param node
     * @param mapdata
     * @return
     */
    private static MeshPolygon/*MeshWayConnector*//*7.3.26SceneryWayConnector*/ createAndPersistConnector(MapNode node, /*19.2.26SceneryObjectList roadsAndBridges*/ TerrainMesh tm, MapData mapdata) throws MeshInconsistencyException, OsmProcessException {
        TagGroup tags = node.getOsmNode().tags;

        log.debug("Creating new connector " + node.getOsmId());
        List<MapWaySegment2> connectedRoads = getConnectedWays(node, mapdata);
        if (connectedRoads.size() == 0) {
            // no way? strange.
            log.warn("no ways at connector " + node.getOsmId());
            return null;
        }
        if (connectedRoads.size() == 1) {
            // unconnected enter node->no connector
            log.warn("Not building one way connector at " + node.getOsmId());
            return null;
        }
        ...
        //19.2.26 roadsAndBridges.add(connector);

        connector.classify();
        // done in persist connector.createPolygon(null, null,  SceneryContext.getInstance());
        // DB persist
        connector.cca(tm, SceneryContext.getInstance());//addToTerrainMesh(tm);
     ...
     * <p>
     * 19.2.26:Deprecated in favour of getConnectedWays() on mapdata level
     *
    ...
    /**
     * 19.2.26 a new approach
     */
    public static List<MapWaySegment2> getConnectedWays(MapNode node,
                                                        MapData mapData) {


        List<MapWay> connectedRoadsWithLanes = new ArrayList<>();

        return mapData.findMapWaySegmentsAtNode(node.getOsmId());
