...
public class SceneryMesh {
   ...
   /* public void clipNonWays() {
    ...
    public MeshFillCandidate[] createWayToAreaCandidates(SceneryWayObject way, TerrainMesh tm) {
        ...
        MeshLine targetLine;
        if ((c0 = JtsUtil.findClosestWithinDistance(rightline.getFrom().getCoordinate(), polycoors, maxdistance)) == -1) {
            return null;
        }
        int c1;
        if ((c1 = JtsUtil.findClosestWithinDistance(rightline.getTo().getCoordinate(), polycoors, maxdistance)) == -1) {
            return null;
        }
