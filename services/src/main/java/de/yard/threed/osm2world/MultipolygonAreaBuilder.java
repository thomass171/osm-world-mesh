...
import static de.yard.threed.osm2world.GeometryUtil.getLineSegmentIntersection;
import static de.yard.threed.osm2world.GeometryUtil.isRightOf;
...
public class MultipolygonAreaBuilder {
	...
	public static final Collection<MapArea> createAreasForMultipolygon(
			OSMRelation relation, Map<OSMNode, MapNode> nodeMap) throws MapAreaCreateException {
		...
		for (OSMMember member : relation.relationMembers) {
			if (member.member instanceof OSMWay) {
				...
				if ("inner".equals(member.role)) {
					...
					List<MapNode> hole = new ArrayList<MapNode>(way.getNodes().size());
					
					for (OSMNode node : ((OSMWay)member.member).getNodes()) {
					...
					outerNodes = new ArrayList<MapNode>(way.getNodes().size());
	...
	private static final class NodeSequence extends ArrayList<MapNode> {
		...
		public NodeSequence(OSMWay way, Map<OSMNode, MapNode> nodeMap) {
			...
			super(way.getNodes().size());
			
			for (OSMNode wayNode : way.getNodes()) {
